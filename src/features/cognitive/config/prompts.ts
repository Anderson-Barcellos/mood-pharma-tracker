/**
 * ### 🧠 Cognitive Test Prompt Configuration
 *
 * Configuração de prompts para geração de Matrizes Progressivas de Raven via Gemini API.
 * Baseado em prototypes/cognitive-test/config.js
 *
 * ### 📚 Notes
 * - Esta configuração é duplicada no backend (api/generate-matrix.js)
 * - O backend é a fonte de verdade para geração de matrizes
 * - Este arquivo serve como referência e documentação para o frontend
 *
 * ### 🔄 Returns
 * - Configurações de prompt por nível de dificuldade
 */
...
export const PROMPTS: Record<'normal' | 'difficult', PromptConfig> = {
  normal: {
 ...
 * ### 🔨 buildGeminiPrompt
 *
 * Constrói o prompt completo para a API do Gemini
 *
 * ### 🖥️ Parameters
 * - `difficulty` (`'normal' | 'difficult'`): Nível de dificuldade da matriz
 *
 * ### 🔄 Returns
 * - `string`: Prompt formatado completo
 *
 * ### 💡 Example
 * ```typescript
 * const prompt = buildGeminiPrompt('normal');
 * ```
 */
export function buildGeminiPrompt(difficulty: 'normal' | 'difficult' = 'normal'): string {
